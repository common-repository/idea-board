...
<table class="form-table idea-board-admin-table">
	...
	<tr>
		...
		<td>
			...
			<p>
				<label for="board_use_forum">
					<input type="checkbox" name="board_use_forum" id="board_use_forum" value="1"
						<?php echo Setting::get_use_forum( $board->term_id, false ) ? 'checked' : '' ?> />

					<?php _e_idea_board( 'Use the bulletin board to update the reference alignment. (In the form of a general forum)<em>If you use a bulletin board Updates Sort reply can not be created, you can create only comment.<br/>If you uncheck this recent article is aligned with standards written reply will be activated</em>' ); ?>
				</label>
			</p>
		</td>
	</tr>
			...
			<p>
				<label for="board_use_comment_skin">
					<input type="checkbox" name="board_use_comment_skin" id="board_use_comment_skin" value="1"
						<?php echo Setting::get_use_comment_skin( $board->term_id, true ) == 1 ? 'checked' : '' ?> />
